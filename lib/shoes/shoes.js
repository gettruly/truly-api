...
module.exports = {
  ...
  get : function(req, res){
    ...
    var query = 'SELECT * FROM ' + SHOES_TBL + ' WHERE ref = ?';
    connection.query(query, ref, function(err, rows, fields) {
      if (err) {
        res.send(500, err);
      }
      res.send(rows[0]);//return 1st value. the only one hopefully
    });
  },
